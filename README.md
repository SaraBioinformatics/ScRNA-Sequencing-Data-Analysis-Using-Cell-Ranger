...
```R
counts <- Read10X(data.dir = "Downloads/DS1/")
...
```R
library(Matrix)
counts <- readMM("Downloads/DS1/matrix.mtx.gz")
barcodes <- read.table("Downloads/DS1/barcodes.tsv.gz", stringsAsFactors=F)[,1]
features <- read.csv("Downloads/DS1/features.tsv.gz", stringsAsFactors=F, sep="\t", header=F)
